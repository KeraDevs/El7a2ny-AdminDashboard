...
const Users: React.FC = () => {
  ...
  const handleAddUser = async (userData: Partial<User>) => {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to add User! kindly share status: ${response.status} with your Techincal team`
        );
      }
    ...
    <Box className="p-6">
      ...
      <Box className="mb-6">
        <Tabs value={tabValue} onChange={(_, value) => setTabValue(value)}>
          <Tab label="Users" icon={<PersonAdd />} iconPosition="start" />
